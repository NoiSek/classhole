...
def dice(inp):
    ...
    try: # if inp is a re.match object...
        ...
        try:
            ...
            return "INVALID INPUT MOTHERFUCKER"
    ...
    if desc:
        return "%s, Average(%0.2f) Total(%d) Rolled(%s=%s)" % (int(desc.strip()), average, total, inp, ", ".join(rolls))
    else:
        return "Average(%0.2f) Total(%d) Rolled(%s=%s)" % (average, total, inp, ", ".join(rolls))
